...
const PythonLesson4 = ({ navigation }) => {
	const insets = useSafeAreaInsets();
	const { markLessonComplete } = useProgress();
	
	const handleLessonComplete = (isCorrect) => {
		if (isCorrect) {
			markLessonComplete('python', 4);
			console.log('Python Lesson 4 completed!');
		}
	};
				...
				<View className="flex-1 px-6 py-5">
					...
					<Text className="text-3xl font-bold mb-2 text-slate-900">
						...
						<LessonCard 
							title="Variables"
							lessonContent={
								"Concept: Variables store values so you can reuse them.\n\n" +
								"Example:\nage = 20\n\n" +
								"Key Point: Python figures out the type automatically.\n\n" +
