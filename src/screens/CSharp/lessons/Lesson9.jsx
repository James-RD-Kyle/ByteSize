...
const CSharpLesson9 = ({ navigation }) => {
	const insets = useSafeAreaInsets();
	const { markLessonComplete } = useProgress();
	
	const handleLessonComplete = (isCorrect) => {
		if (isCorrect) {
			markLessonComplete('csharp', 9);
			console.log('C# Lesson 9 completed!');
		}
	};
				...
				<View className="flex-1 px-6 py-5">
					...
					<Text className="text-3xl font-bold mb-2 text-slate-900">
						...
						<LessonCard 
							title="If Statements"
							lessonContent={
								"Concept: An if statement runs code only when the condition is true.\n\n" +
